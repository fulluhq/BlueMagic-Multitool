...
option_32 = "Discord-Rat-Builder-(Premium)"
option_33 = "Ransomware-Builder-(Premium)"
option_34 = "Website-DoS-(Premium)"
option_35 = "Soon"
option_36 = "Soon"
option_37 = "Soon"
option_38 = "Soon"
...
def Update():
      popup_version = ""
      try:
         response = requests.get(f"{url_api}/version.php")
         version_now = response.json()['version']
         if not version_now == version_tool:
            colorama.init()
            input(f"{BEFORE + current_time_hour() + AFTER} {INFO} Please install the new version of the tool: {white + version_tool + red} -> {white + version_now} ")
            popup_version = f"{red}New Version: {white + version_tool + red} -> {white + version_now}"
            colorama.deinit()
            Clear()
      except: pass
 ...
 └─{white}$ {reset}"""))
      ...
      if choice in ['N', 'n', 'NEXT', 'Next', 'next']:
         ...
         try:
            ...
            file_path = os.path.join(tool_path, "Program", "FileDetectedByAntivirus", "BuilderOptions.py")
